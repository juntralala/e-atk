...
function handleSubmit() {
  form.put(route('stock.adjustments.reasons.update', props.opnameReasonId), {
    preserveScroll: true,
    onSuccess() {
      closeFormDialog();
    },
  ...
  <v-list-item value="edit">
    ...
    <v-dialog activator="parent" v-model="showFormDialog">
      <v-form @submit.prevent="handleSubmit">
        <v-card>
          <v-card-title class="bg-blue-darken-2 text-center">Edit Sebab Penyesuaian Stok</v-card-title>
          <v-card-text>
