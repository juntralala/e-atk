...
const applyFilter = () => {
  router.get(
    route('items.requests.exports.view'),
    {
...
const tableData = props.itemRequests.data.flatMap((request, requestIndex) => {
  return request.item_request_details.map((detail, detailIndex) => {
    const globalIndex =
      props.itemRequests.data.slice(0, requestIndex).reduce((sum, r) => sum + r.item_request_details.length, 0) +
      detailIndex;
        ...
        <v-data-table
          ...
          <template #bottom v-if="false">
            <tr class="bg-blue-lighten-5 font-weight-bold">
              <td colspan="11" class="pa-4 text-right">Total (berdasarkan permintaan diterima):</td>
