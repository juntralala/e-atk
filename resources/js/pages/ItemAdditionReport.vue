...
const tableData = props.itemAdditions.data.flatMap((addition, additionIndex) => {
  return addition.item_addition_details.map((detail, detailIndex) => {
    const globalIndex = props.itemAdditions.data.slice(0, additionIndex).reduce((sum, a) => sum + a.item_addition_details.length, 0) + detailIndex;
    ...
    <v-row>
      <v-col>
        <PageTitleHighlightPart
          ...
          second-part-title="Penambahan Barang"
        />
      </v-col>
    </v-row>
    ...
    <v-row class="mt-4">
      <v-col>
        ...
        <v-row v-if="!tableData || tableData.length === 0">
          <v-col>
            <v-card>
              <v-card-text class="text-grey pa-8 text-center"> Tidak ada data penambahan barang pada periode ini </v-card-text>
            </v-card>
          </v-col>
        </v-row>
      </v-col>
