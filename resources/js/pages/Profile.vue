...
const cancelEdit = () => {
  ...
  editForm.value = { name: '', username: '', password: '', password_confirmation: '', photo: '', photoFile: null, photoPreview: '', telepon: '' };
  errorMessage.value = '';
  successMessage.value = '';
  validationErrors.value = {};
};
...
const handleFileChange = (event) => {
  ...
  if (!file) return;
  const allowedTypes = ['image/jpeg', 'image/jpg', 'image/png', 'image/webp'];
  if (!allowedTypes.includes(file.type)) { validationErrors.value.photo = 'Format foto harus JPG, PNG, atau WEBP'; return; }
  const maxSize = 2 * 1024 * 1024;
  if (file.size > maxSize) { validationErrors.value.photo = 'Ukuran foto maksimal 2MB'; return; }
  delete validationErrors.value.photo;
  editForm.value.photoFile = file;
  const reader = new FileReader();
  reader.onload = (e) => { editForm.value.photoPreview = e.target.result; };
  reader.onerror = () => { validationErrors.value.photo = 'Gagal membaca file'; };
      ...
      <v-row>
        ...
        <v-col cols="12" md="4" lg="3">
          <div class="left-panel">
            ...
            <div
              class="avatar-zone"
              :class="{ 'avatar-zone--editable': isEditing }"
            ...
            <input ref="fileInput" type="file" accept="image/jpeg,image/jpg,image/png,image/webp" style="display:none" @change="handleFileChange" />
            ...
            <div class="identity">
              ...
              <div class="role-pill">
                <span class="role-dot"></span>{{ profile.role }}
              </div>
            </div>
        ...
        <v-col cols="12" md="8" lg="9">
          <div class="right-panel">
            ...
            <div class="panel-header">
              ...
              <div v-if="isEditing" class="edit-badge">
                <v-icon size="13">mdi-pencil</v-icon> Mode Edit
              </div>
            </div>
            ...
            <div v-else class="edit-fields">
              <div class="input-wrap">
                ...
                <input v-model="editForm.name" class="inp" :class="{ 'inp--err': validationErrors.name }" :disabled="processing" placeholder="Nama lengkap" />
              ...
              <div class="input-wrap">
                ...
                <input v-model="editForm.username" class="inp" :class="{ 'inp--err': validationErrors.username }" :disabled="processing" placeholder="username_kamu" />
              ...
              <div class="input-wrap">
                ...
                <div class="inp-relative">
                  <input v-model="editForm.password" :type="showPassword ? 'text' : 'password'" class="inp inp--padded" :class="{ 'inp--err': validationErrors.password }" :disabled="processing" placeholder="Kosongkan jika tidak diubah" />
                  <button type="button" class="eye-btn" @click="showPassword = !showPassword">
                    <v-icon size="17" color="#94a3b8">{{ showPassword ? 'mdi-eye-off-outline' : 'mdi-eye-outline' }}</v-icon>
                  </button>
                </div>
              ...
              <div v-if="editForm.password" class="input-wrap">
                ...
                <input v-model="editForm.password_confirmation" :type="showPassword ? 'text' : 'password'" class="inp" :class="{ 'inp--err': validationErrors.password_confirmation }" :disabled="processing" placeholder="Ulangi password baru" />
                <p v-if="validationErrors.password_confirmation" class="err-text">{{ validationErrors.password_confirmation }}</p>
              </div>
              ...
              <div class="input-wrap">
                ...
                <input v-model="editForm.telepon" class="inp" :class="{ 'inp--err': validationErrors.telepon }" :disabled="processing" placeholder="08xxxxxxxxxx" />
...
.profile-container { max-width: 1000px !important; }
...
.avatar-zone--editable { cursor: pointer; }
...
.avatar-initials { font-size: 36px; font-weight: 700; color: #2563eb; }
...
.avatar-zone--editable:hover .avatar-hover-mask { opacity: 1; }
...
.identity-name { font-size: 17px; font-weight: 700; color: #0f172a; margin: 0 0 2px; }
.identity-username { font-size: 13px; color: #94a3b8; margin: 0 0 12px; }
...
.btn-edit-main:hover { background: #1d4ed8; transform: translateY(-1px); }
...
/* VIEW FIELDS */
.view-fields { display: flex; flex-direction: column; }
.field-item { display: flex; align-items: flex-start; gap: 14px; padding: 16px 4px; }
.field-icon {
...
.field-val { font-size: 15px; font-weight: 500; color: #0f172a; margin: 0; }
.field-val.pw-mask { letter-spacing: 3px; color: #64748b; }
.field-val.field-empty { color: #cbd5e1; font-style: italic; }
.field-sep { height: 1px; background: #f8fafc; margin: 0 4px; }
...
.inp:focus { border-color: #3b82f6; background: white; box-shadow: 0 0 0 3px rgba(191,219,254,0.5); }
.inp--err { border-color: #fca5a5; background: #fef2f2; }
.inp:disabled { opacity: 0.55; cursor: not-allowed; }
.inp--padded { padding-right: 44px; }

.inp-relative { position: relative; }
.eye-btn {
...
.err-text { font-size: 12px; color: #ef4444; font-weight: 500; margin: 5px 0 0; }
.hint-text { font-size: 12px; color: #94a3b8; margin: 5px 0 0; }
...
.btn-save { background: #2563eb; color: white; box-shadow: 0 2px 12px rgba(37,99,235,0.28); }
.btn-save:hover:not(:disabled) { background: #1d4ed8; box-shadow: 0 4px 16px rgba(37,99,235,0.38); transform: translateY(-1px); }
